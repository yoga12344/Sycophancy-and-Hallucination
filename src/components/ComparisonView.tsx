      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div>
              ...
              <div className="mt-4 space-y-2 text-xs">
                {!isIntervened && currentAnalysis.risk?.level === 'LOW' ? (
                  <div className="flex items-start space-x-2 text-emerald-300 bg-emerald-500/10 border border-emerald-500/20 p-2.5 rounded-lg">
                  ...
                  <>
                    <div className="flex items-start space-x-2 text-rose-300">
                      ...
                      <span><strong className="text-zinc-200">Validation Marker:</strong> {currentAnalysis.sycophancy?.reasoning || 'Response endorsed the user premise without requiring empirical evidence.'}</span>
                    </div>
                    <div className="flex items-start space-x-2 text-rose-300">
                      <X className="w-3.5 h-3.5 text-rose-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Evidence Omission:</strong> {currentAnalysis.evidenceBalance?.explanation || 'Countervailing empirical literature was omitted in favor of confirmation.'}</span>
                    </div>
                    <div className="flex items-start space-x-2 text-rose-300">
                      <X className="w-3.5 h-3.5 text-rose-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Belief Trajectory:</strong> {currentAnalysis.trajectory?.explanation || 'Unqualified validation accelerates user confidence toward dogmatic fixation.'}</span>
                    </div>
                  </>
                )}
              </div>
          ...
          <div className="bg-zinc-900/80 rounded-2xl border border-emerald-900/40 p-5 flex flex-col justify-between shadow-xs relative overflow-hidden">
                ...
                {isIntervened ? (
                  <>
                    <div className="flex items-start space-x-2 text-emerald-300">
                      <Check className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Calibrated Boundaries:</strong> {currentAnalysis.intervention?.reason}</span>
                    </div>
                    <div className="flex items-start space-x-2 text-emerald-300">
                      <Check className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Multi-Perspective Evidence:</strong> Balanced confirmation by providing peer-reviewed countervailing studies and empirical limits.</span>
                    </div>
                    <div className="flex items-start space-x-2 text-emerald-300">
                      <Check className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Spiral Interruption:</strong> Replaced dogmatic validation with grounded uncertainty and falsification criteria.</span>
                    </div>
                  </>
                ) : (
                  <>
                    <div className="flex items-start space-x-2 text-emerald-300">
                      <Check className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Epistemic Integrity Verified:</strong> The model draft was evaluated and determined to be objective and unmanipulated.</span>
                    </div>
                    <div className="flex items-start space-x-2 text-emerald-300">
                      <Check className="w-4 h-4 text-emerald-400 shrink-0 mt-0.5" />
                      <span><strong className="text-zinc-200">Zero Unnecessary Rewriting:</strong> Clean pass without altering natural conversational flow.</span>
                    </div>
                  </>
                )}
              </div>
            ...
            <div className="mt-6 pt-4 border-t border-zinc-800 grid grid-cols-3 gap-2 text-center text-xs font-mono">
              <div className="bg-zinc-950 p-2 rounded border border-zinc-800">
                <span className="text-[10px] text-zinc-500 block">Sycophancy Filter</span>
                <span className="font-semibold text-emerald-400">PASSED</span>
              </div>
              <div className="bg-zinc-950 p-2 rounded border border-zinc-800">
                <span className="text-[10px] text-zinc-500 block">Evidence Balance</span>
        ...
        <div className="mt-8 bg-zinc-900 border border-zinc-800 rounded-2xl p-5 shadow-xs">
          <h3 className="text-sm font-semibold text-zinc-100 mb-3 flex items-center space-x-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 pt-2">
            <div className="p-3 bg-zinc-950 rounded-lg border border-zinc-800">
              <span className="text-[10px] font-mono text-zinc-400 font-semibold block mb-1 uppercase tracking-wider">
